...
WHERE up.status = 'Active' $filter_query
...
.filter-bar {
    ...
    margin: 20px 0;
        ...
        .badge { 
            background: #4cd137; color: white; padding: 4px 10px; border-radius: 12px; font-size: 0.8em; 
        }
...
<?php foreach ($users as $name => $user): ?>
    ...
    <span class="material-icons">person</span>
            ...
            <div class="info">
                ...
                <p>ID: <?= $user['employee_id'] ?> • <?= $user['department'] ?> • <?= $user['role'] ?> • <span class="badge"><?= $user['status'] ?></span></p>
            </div>
